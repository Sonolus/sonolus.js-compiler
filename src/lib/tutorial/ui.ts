...
const uiPointer = (x: number, index: number) => preprocessWritablePointer(1005, x, index, 9)
...
const configurationPointer = (x: number, index: number) =>
    preprocessWritablePointer(1006, x, index, 2)
...
export const ui = {
    ...
    instruction: createUI(3),

    configuration: {
        menu: createUIConfiguration(0),
        navigation: createUIConfiguration(1),
        instruction: createUIConfiguration(2),
    },
} as const
